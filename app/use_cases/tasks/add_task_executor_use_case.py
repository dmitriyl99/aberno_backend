...
class AddTaskExecutorUseCase:
    ...
    def execute(self, task_id: int, employee_id: int) -> Task:
        ...
        with self.session() as session:
            ...
            executor_exists = len(list(filter(lambda e: e.employee_id == employee_id, task.executors))) > 0
            if executor_exists:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail='This executor already involved in this task'
                )
